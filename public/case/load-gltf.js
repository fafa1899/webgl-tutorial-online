...
async function main() {
  //读取gltf文件
  try {
    let response = await fetch('new.gltf');  
    if (!response.ok) {
      throw new Error("HTTP error " + response.status);
    }
    const gltfObj = await response.json();

    //读取bin文件
    response = await fetch(gltfObj.buffers[0].uri);  
    if (!response.ok) {
      throw new Error("HTTP error " + response.status);
    }
    const binObj = await response.arrayBuffer();
    ...
    image.onload = function () {
      //绘制函数
      onDraw(gl, canvas, gltfObj, binObj, image);
    };

    //浏览器开始加载图像
    image.src = gltfObj.images[0].uri;   
  } catch (error) {
